...
- **안정적인 추론 처리**: YOLO와 OneOCR 호출은 전용 워커 스레드에서 순차 실행되어 DLL/모델 동시 호출 충돌을 줄입니다.
- **Fallback 스캔**: YOLO 탐지 영역을 우선 처리하고, 탐지 실패 또는 오검출에 대비해 전체 이미지 OCR을 마지막 후보로 다시 시도합니다.
...
- **Windows 완료 알림**: GUI 모드에서는 분석 완료 시 선택적으로 Windows 메시지 알림을 표시합니다.
...
- GUI 저장은 Excel 파일을 먼저 만들고, 같은 결과를 SQLite 이력 DB에 추가로 동기화합니다.
- 작업 완료 알림은 `설정`의 Windows 알림 옵션에서 켜거나 끌 수 있습니다.
...
웹 모드는 메인 Excel을 덮어쓰기 전에 날짜별 백업 파일을 먼저 생성합니다. 메인 Excel이 열려 있어 덮어쓰지 못해도 백업 파일과 화면 안내를 통해 저장 실패 범위를 확인할 수 있습니다.
